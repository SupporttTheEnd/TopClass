...
jQuery(document).ready(function () {
    ...
    async function hrefClickHandler(
        ...
        var userData = {
            ...
            UserName: TCRealName && TCRealName.trim() ? TCRealName.trim().split(/\s+/)[0] + " " + TCRealName.trim().split(/\s+/).slice(-1) : ""
        };
...
if (!currentUserType.includes("Admin") && !currentUserType.includes("Manager")) {
    (function () {
        ...
        function handleAdminMenu() {
            ...
            // Remove admin items from navbar
            adminItems.forEach((item) => {
                if (item.parentNode === navbar) navbar.removeChild(item);
            });
